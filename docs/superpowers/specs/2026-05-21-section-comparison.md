...
| 8 | Full row-set + H2 + intro + footnote rewritten | 2026-05-14 lock (`docs/content/homepage.md`) | A3 chain: user reopened the lock — *"too aggressive — make it professional and SEO optimised"*. Baseline 5.2 → rewrite 8.4 (see `docs/superpowers/specs/2026-05-21-comparison-copy-audit.md`). |
...
> 2026-05-21 | Comparison redesigned (no Figma; baseline = shipped 47-line JSX): migrated to canonical `<Section bg="default" maxWidth="xwide">` grid (was hand-rolled 4-step padding on `bg-bg-subtle`, max-w-960); eyebrow + Section primitives consumed; table gains a11y (`scope="col" / scope="row"`, `sr-only <caption>`, `Dimension` sr-only header); striping migrated to `bg-bg-subtle/60` against the new white section bg; footnote + competitor cells `text-gray-light` → `text-gray` (DESIGN.md line 248 compliance). **Full A3 copy rewrite** (2026-05-14 lock reopened by user — *"too aggressive, make it professional and SEO optimised"*): H2 expanded with comparison-intent keyword chain (`How Metaborong compares to large agencies and freelance teams`); intro reframed as 24w factual lede; 6 row labels + 18 cells rewritten as parallel professional noun phrases; `7 chains — Ethereum, Solana, Base, Arbitrum, Hyperledger, Polygon, Avalanche` preserved verbatim; `DeFi · AI · SaaS shipped` → `25+ products in production` (D5, user-verified). Baseline 5.2 → rewrite 8.4 (`docs/superpowers/specs/2026-05-21-comparison-copy-audit.md`). Cross-file flag for future session: TRUST SIGNALS `8+ products` drifts against `25+`. Deviations 1–8 logged in `docs/superpowers/specs/2026-05-21-section-comparison.md`. | Section grammar now matches nav / hero / why-us / founders / contact-cta. Table is citation-ready for AEO. Voice matches Founders + Why-Us. |

**CHANGELOG.md entry (Session 18 sub-bullet):**

> - **Comparison** — A2 + A3 pass. Migrated to canonical Section grid (`bg="default" maxWidth="xwide"`); table gains a11y (`scope` + `<caption>`); full copy rewrite — H2 + intro + 6 rows × 3 columns, all row labels renamed (`DeFi depth` → `Multichain coverage`; `Speed to delivery` → `Delivery timeline`; `Product thinking` → `Product strategy`; `AI-native services` → `AI engineering depth`); `Track record` → `25+ products in production` (was `DeFi · AI · SaaS shipped`); 7-chain string preserved verbatim. A3 baseline 5.2 → 8.4.

**A3 copy-audit scorecard row (orchestrator graduation):**

| Section | Baseline | Post-rewrite | Δ |
|---|---|---|---|
| Comparison (Session 18) | 5.2 | 8.4 | +3.2 |
